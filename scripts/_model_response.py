...
class ChatResponse(DictMixin):
    ...
    """The token IDs for each generated token in the response.
    Each inner list contains the byte-level token IDs for one token."""

    log_probs: list[float] | None = field(default_factory=lambda: None)
    """The log probabilities for each generated token in the response."""
