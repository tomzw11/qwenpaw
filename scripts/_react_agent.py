...
class ReActAgent(ReActAgentBase):
    ...
    async def _reasoning(
        ...
        # Collect token_ids and log_probs across all chunks
            ...
            async with tts_context:
                ...
                if self.model.stream:
                    async for content_chunk in res:
                        ...
                        # Collect token_ids and log_probs from each chunk
                        chunk_token_ids = getattr(
                            content_chunk, "token_ids", None,
                        )
                        chunk_log_probs = getattr(
                            content_chunk, "log_probs", None,
                        )
                        if chunk_token_ids:
                    ...
                    all_token_ids = getattr(res, "token_ids", None) or []
                    all_log_probs = getattr(res, "log_probs", None) or []

                # Store token_ids and log_probs in msg metadata
                if all_token_ids or all_log_probs:
                    if msg.metadata is None:
                        msg.metadata = {}
                    msg.metadata["token_ids"] = all_token_ids
                    msg.metadata["log_probs"] = all_log_probs
                    logger.info(
                        "Step token_ids count: %d, log_probs count: %d",
                        len(all_token_ids),
                        len(all_log_probs),
                    )
        ...
        async with tts_context:
            ...
            if isinstance(res, AsyncGenerator):
                async for chunk in res:
                    ...
                    chunk_token_ids = getattr(chunk, "token_ids", None)
                    chunk_log_probs = getattr(chunk, "log_probs", None)
                    if chunk_token_ids:
                        all_token_ids.extend(chunk_token_ids)
                    if chunk_log_probs:
                        all_log_probs.extend(chunk_log_probs)
            ...
            if all_token_ids or all_log_probs:
                if res_msg.metadata is None:
                    res_msg.metadata = {}
                res_msg.metadata["token_ids"] = all_token_ids
                res_msg.metadata["log_probs"] = all_log_probs
