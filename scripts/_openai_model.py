...
class OpenAIChatModel(ChatModelBase):
    ...
    async def _parse_openai_stream_response(
        ...
        last_contents = None
        collected_token_ids: list[list[int]] = []
        collected_log_probs: list[float] = []
            ...
            async for item in stream:
                ...
                if not chunk.choices:
                    if usage and contents:
                        _kwargs: dict[str, Any] = {
                            ...
                            "usage": usage,
                            "metadata": metadata,
                            "token_ids": collected_token_ids or None,
                            "log_probs": collected_log_probs or None,
                        }
                        if response_id:
                ...
                if logprobs_content:
                    for token_logprob in logprobs_content:
                        token_bytes = getattr(token_logprob, "bytes", None)
                        if token_bytes is not None:
                            collected_token_ids.append(list(token_bytes))
                        collected_log_probs.append(
                            getattr(token_logprob, "logprob", 0.0),
                        )
        ...
        if response.choices:
            ...
            if logprobs_content:
                ...
                for token_logprob in logprobs_content:
                    token_bytes = getattr(token_logprob, "bytes", None)
                    if token_bytes is not None:
                        token_ids.append(list(token_bytes))
                    log_probs.append(
                        getattr(token_logprob, "logprob", 0.0),
                    )
